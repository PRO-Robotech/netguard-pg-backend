...
if [[ -z "${SKIP_OPENAPI:-}" ]]; then
  if command -v openapi-gen >/dev/null 2>&1; then
    ...
    if [[ -z "${VERSION_PKGS}" ]]; then
      ...
      # openapi-gen (v0.33.x) takes the list of input packages as positional
      # arguments – no --input-dirs flag. We pass our API version packages first,
      # then the standard k8s packages.
