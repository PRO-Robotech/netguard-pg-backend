...
func (m *MockReaderForIEAgAgRuleValidator) ListNetworks(ctx context.Context, consume func(models.Network) error, scope ports.Scope) error {
	return nil
}

func (m *MockReaderForIEAgAgRuleValidator) ListNetworkBindings(ctx context.Context, consume func(models.NetworkBinding) error, scope ports.Scope) error {
	return nil
}

func (m *MockReaderForIEAgAgRuleValidator) GetNetworkByID(ctx context.Context, id models.ResourceIdentifier) (*models.Network, error) {
	return nil, fmt.Errorf("network not found")
}

func (m *MockReaderForIEAgAgRuleValidator) GetNetworkBindingByID(ctx context.Context, id models.ResourceIdentifier) (*models.NetworkBinding, error) {
...
func TestIEAgAgRuleValidator_ValidateForUpdate(t *testing.T) {
	...
	// Create old and new rules (oldRule в состоянии Ready для проверки immutable полей)
	...
	// Test valid update (only changing mutable fields: ports, logs, priority)
...
// TestIEAgAgRuleValidator_ImmutableFields tests that immutable fields are protected when rule is Ready
func TestIEAgAgRuleValidator_ImmutableFields(t *testing.T) {
	mock := &MockReaderForIEAgAgRuleValidator{
		addressGroupLocalExists: true,
		addressGroupLocalID:     "local-ag",
		addressGroupExists:      true,
		addressGroupID:          "target-ag",
	}

	validator := validation.NewIEAgAgRuleValidator(mock)

	// Create rule in Ready state
	readyRule := models.IEAgAgRule{
		SelfRef:   models.NewSelfRef(models.NewResourceIdentifier("test-rule")),
		Transport: models.TransportProtocol("TCP"),
		Traffic:   models.Traffic("INGRESS"),
		AddressGroupLocal: models.AddressGroupRef{
			ResourceIdentifier: models.NewResourceIdentifier("local-ag"),
		},
		AddressGroup: models.AddressGroupRef{
			ResourceIdentifier: models.NewResourceIdentifier("target-ag"),
		},
		Action: models.ActionAccept,
		Ports: []models.PortSpec{
			{Destination: "80"},
	...
	// Test Transport immutable
	modifiedRule := readyRule
	modifiedRule.Transport = models.TransportProtocol("UDP")
	err := validator.ValidateForUpdate(context.Background(), readyRule, modifiedRule)
	if err == nil || !strings.Contains(err.Error(), "Transport field is immutable") {
		t.Errorf("Expected Transport immutable error, got: %v", err)
	}

	// Test Traffic immutable
	modifiedRule = readyRule
	modifiedRule.Traffic = models.Traffic("EGRESS")
	err = validator.ValidateForUpdate(context.Background(), readyRule, modifiedRule)
	if err == nil || !strings.Contains(err.Error(), "Traffic field is immutable") {
		t.Errorf("Expected Traffic immutable error, got: %v", err)
	}

	// Test Action immutable
	modifiedRule = readyRule
	modifiedRule.Action = models.ActionDrop
	err = validator.ValidateForUpdate(context.Background(), readyRule, modifiedRule)
	if err == nil || !strings.Contains(err.Error(), "Action field is immutable") {
		t.Errorf("Expected Action immutable error, got: %v", err)
	}

	// Test AddressGroupLocal immutable
	modifiedRule = readyRule
	modifiedRule.AddressGroupLocal.ResourceIdentifier = models.NewResourceIdentifier("different-ag")
	err = validator.ValidateForUpdate(context.Background(), readyRule, modifiedRule)
	if err == nil || !strings.Contains(err.Error(), "AddressGroupLocal field is immutable") {
		t.Errorf("Expected AddressGroupLocal immutable error, got: %v", err)
	}

	// Test that non-Ready rule allows changes
	nonReadyRule := readyRule
	nonReadyRule.Meta.Conditions = []metav1.Condition{
		{
			Type:   "Ready",
			Status: "False",
		},
	}
	modifiedRule = nonReadyRule
	modifiedRule.Transport = models.TransportProtocol("UDP")
	err = validator.ValidateForUpdate(context.Background(), nonReadyRule, modifiedRule)
	if err != nil {
		t.Errorf("Expected no error for non-Ready rule modification, got: %v", err)
	}
}
