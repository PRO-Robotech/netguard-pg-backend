...
func TestIntegration_AddressGroupDependencies(t *testing.T) {
	...
	addressGroupID := models.ResourceIdentifier{Name: "test-address-group", Namespace: "default"}
	addressGroup := models.AddressGroup{
		SelfRef: models.SelfRef{ResourceIdentifier: addressGroupID},
	}

	serviceID := models.ResourceIdentifier{Name: "test-service", Namespace: "default"}
	...
	// Create reader AFTER commit to see the committed data
	reader, err := registry.Reader(context.Background())
	if err != nil {
		t.Fatalf("Failed to get reader: %v", err)
	}
