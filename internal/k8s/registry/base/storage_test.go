...
func TestBaseStorage_ApplyPatch_SupportedTypes(t *testing.T) {
	...
	// JSON patch with empty data should error
	_, err := storage.applyPatch(obj, types.JSONPatchType, []byte{})
	if err == nil {
		t.Error("Expected error for empty JSON patch data")
	}

	// Merge patch with empty data should error
	_, err = storage.applyPatch(obj, types.MergePatchType, []byte{})
	if err == nil {
		t.Error("Expected error for empty merge patch data")
	}

	// Strategic merge patch with empty data should error
	_, err = storage.applyPatch(obj, types.StrategicMergePatchType, []byte{})
	if err == nil {
		t.Error("Expected error for empty strategic merge patch data")
	}
}
...
func TestBaseStorage_ApplyServerSide_EmptyPatch(t *testing.T) {
	storage := createTestStorage()
	ctx := createTestContext("test-ns")

	_, err := storage.ApplyServerSide(ctx, "test-name", []byte{}, "test-manager", false)
	if err == nil {
		t.Fatalf("expected error for empty server-side apply patch, got nil")
	}
}

func TestBaseStorage_ApplyServerSide_Success(t *testing.T) {
	storage := createTestStorage()
	ctx := createTestContext("test-ns")

	patch := []byte(`{"spec":{"value":"new"}}`)
	obj, err := storage.ApplyServerSide(ctx, "test-name", patch, "test-manager", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	mock, ok := obj.(*MockK8sObject)
	if !ok {
		t.Fatalf("expected *MockK8sObject, got %T", obj)
	}

	if mock.Spec.Value != "new" {
		t.Errorf("expected spec.value to be 'new', got %q", mock.Spec.Value)
	}
}
