	...
	testCases := []struct {
	...
	}{
		{
			...
			checkFunc: func(t *testing.T, result *netguardv1beta1.IEAgAgRule) {
				...
				assert.Equal(t, "TCP", result.Spec.Transport)
				assert.Equal(t, "Ingress", result.Spec.Traffic)
		...
		},
		{
			...
			checkFunc: func(t *testing.T, result *netguardv1beta1.IEAgAgRule) {
				...
				assert.Equal(t, "UDP", result.Spec.Transport)
				assert.Equal(t, "Egress", result.Spec.Traffic)
