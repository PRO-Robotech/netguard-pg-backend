...
// NetworkFieldLabelConversion validates and converts field labels for Network resources.
...
// in kubectl queries like: kubectl get networks --field-selector=status.isBound=true
//
// Supported fields:
...
func NetworkFieldLabelConversion(label, value string) (string, string, error) {
	switch label {
	case "metadata.name",
		"metadata.namespace",
		"status.isBound",
	...
	default:
		...
		return "", "", fmt.Errorf("field label %q not supported for Network: only metadata.name, metadata.namespace, status.isBound, status.addressGroupRef.name, status.addressGroupRef.namespace, status.bindingRef.name, status.bindingRef.namespace are supported", label)
	}
}
