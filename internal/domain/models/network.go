...
func (n *Network) ToSGroupsProto() (interface{}, error) {
	...
	// Build network name with namespace if present
	networkName := n.Name
	if n.Namespace != "" {
		networkName = fmt.Sprintf("%s/%s", n.Namespace, n.Name)
	}
	...
	// Return single network element (not wrapped in SyncNetworks)
	// Batch aggregation will be handled by NetworkSyncer.SyncBatch()
