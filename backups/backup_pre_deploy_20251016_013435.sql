...
BEGIN
    IF TG_OP = 'INSERT' THEN
        v_operation_type := 'CREATE'::sync_operation;
        RAISE NOTICE '[035] HostBinding %.% created, creating outbox entry', NEW.namespace, NEW.name;
    ELSE
        v_operation_type := 'UPDATE'::sync_operation;
        RAISE NOTICE '[035] HostBinding %.% updated, creating outbox entry', NEW.namespace, NEW.name;
    END IF;

    v_resource_id := uuid_generate_v5(
        uuid_ns_dns(),
        'HostBinding:' || NEW.namespace || '/' || NEW.name
    );
    ...
    RAISE NOTICE '[035] ✅ Outbox entry created for HostBinding %.% (resource_id: %)',
        NEW.namespace, NEW.name, v_resource_id;
...
--
-- Name: FUNCTION trigger_host_binding_upsert_outbox(); Type: COMMENT; Schema: public; Owner: netguard
--

COMMENT ON FUNCTION public.trigger_host_binding_upsert_outbox() IS 'Migration 035: Fixed variable naming conflict from Migration 034.';
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        v_operation_type := 'CREATE'::sync_operation;
        RAISE NOTICE '[035] NetworkBinding %.% created, creating outbox entry', NEW.namespace, NEW.name;
    ELSE
        v_operation_type := 'UPDATE'::sync_operation;
        RAISE NOTICE '[035] NetworkBinding %.% updated, creating outbox entry', NEW.namespace, NEW.name;
    END IF;

    v_resource_id := uuid_generate_v5(
        uuid_ns_dns(),
        'NetworkBinding:' || NEW.namespace || '/' || NEW.name
    );
    ...
    RAISE NOTICE '[035] ✅ Outbox entry created for NetworkBinding %.% (resource_id: %)',
        NEW.namespace, NEW.name, v_resource_id;
...
--
-- Name: FUNCTION trigger_network_binding_upsert_outbox(); Type: COMMENT; Schema: public; Owner: netguard
--

COMMENT ON FUNCTION public.trigger_network_binding_upsert_outbox() IS 'Migration 035: Fixed variable naming conflict from Migration 034.';
...
SELECT pg_catalog.setval('public.k8s_metadata_resource_version_seq', 115, true);
...
--
-- Name: TRIGGER trg_host_binding_upsert_outbox ON host_bindings; Type: COMMENT; Schema: public; Owner: netguard
--

COMMENT ON TRIGGER trg_host_binding_upsert_outbox ON public.host_bindings IS 'Migration 034: Unified outbox trigger for HostBinding.
Fires on every INSERT/UPDATE, creates outbox entry for async SGROUP sync.';
...
--
-- Name: TRIGGER trg_network_binding_upsert_outbox ON network_bindings; Type: COMMENT; Schema: public; Owner: netguard
--

COMMENT ON TRIGGER trg_network_binding_upsert_outbox ON public.network_bindings IS 'Migration 034: Unified outbox trigger for NetworkBinding.
Fires on every INSERT/UPDATE, creates outbox entry for async SGROUP sync.';
