-- +goose Up
-- Remove the broad BEFORE UPDATE triggers introduced in migration 108.
-- They caused SQLSTATE 27000 when other triggers updated the same tuple.
...
-- +goose Down
-- Recreate the helper trigger + function (identical to migration 108) in case of rollback.
